#!/usr/bin/env python3
"""Merge app/translations/harbour-helmsman.ts (lupdate template) into per-language files.

For each app/translations/harbour-helmsman_<lang>.ts present: keep existing finished
...
unfinished, drop vanished ones. Run via tools/build-qm.sh, never by hand.
"""
import glob
import copy
import os
...
TPL = os.path.join(REPO, "app", "translations", "harbour-helmsman.ts")
PREFIX = "harbour-helmsman_"


def messages(path):
    """source -> finished translation text."""
    out = {}
...
def main():
    ...
    langs = sorted(
        os.path.basename(p)[len(PREFIX) : -len(".ts")]
        for p in glob.glob(os.path.join(REPO, "app", "translations", f"{PREFIX}*.ts"))
    )
    if not langs:
        print("no language files (app/translations/harbour-helmsman_<lang>.ts); template only")
        return 0

    for lang in langs:
        path = os.path.join(REPO, "app", "translations", f"{PREFIX}{lang}.ts")
        old = messages(path)
